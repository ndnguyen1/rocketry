...
plot(rocketVerticalVelocity)



% Goanna Configuration 
maxAirbrakeArea_Goanna = 0.005352;

maxAirbrakeArea_Skink = 0.001244361;
% % rocketMass = rocketMass/1000;
% % rocketReferenceArea = rocketReferenceArea/10000;
% % Extract time and data from trajectoryPlanner time series
% time = trajectoryPlanner.Time;
% target_velocity = trajectoryPlanner.Data;
% 
% % Plot the main line with thicker width
% plot(time, target_velocity, 'LineWidth', 2);
% 
% % Hold the plot to add dots
% hold on;
% 
% % Define specific times where you want to add dots
% specific_times = [5.3, 7.06, 9.1, 13, 15.6];
% 
% % Find the indices closest to these specific times
% [~, indices] = arrayfun(@(t) min(abs(time - t)), specific_times);
% 
% % Plot dots at these specific times on the curve
% plot(time(indices), target_velocity(indices), 'ro', 'MarkerSize', 5, 'MarkerFaceColor', 'r');
% 
% % Update the title and labels
% title('Trajectory Planner Operating Points by Velocity Region');
% xlabel('Time (seconds)');
