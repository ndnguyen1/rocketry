...
void setup()
{
  ...
  airbrakeControllerV2 = std::make_unique<AirbrakeController>(AIRBRAKE_CONTROLLER_PROFILES::GOANNA_PIL);
}
//
...
void Controller()
{
  ...
  airbrakeControllerV2->update(altitude, velocity);
  const float targetVelocity = airbrakeControllerV2->getTargetVelocity();
  airbrakeControllerV2->computeControlEffort(velocity);
 ...
 // Serial.print(u_k);
  ...
  extra_out1 = altitude / 400.0; //extra_in1
  extra_out2 = velocity / 40.0;  //extra_in2
  extra_out4 = rocketArea; //extra_in4
  extra_out5 = targetVelocity / 60.0; //
  extra_out6 = 0.0;
