...
float calculateCoeffientOfDrag(float vertVelocity, float airDensity, float rocketCrossArea) const;
  ...
  */
  void update(float altitude, float verticalVelocity);
...
//private:
  ...
  */
  void computeOptimumVelocityTrajectory(
    float currentAltitude,
    float airDensity,
    float dragCoefficient);

  /**
   * Calculate the controller effort using the current and target velocity of the rocket
   *
   * @param verticalVelocity Current vertical velocity of the rocket (m/s)
   * 
  */
  void computeControlEffort(float verticalVelocity);

  /**
   * Compute the estimated apogee of the rocket using its current position and velocity
   *
   * @param altitude Altitude of the rocket (m)
   * @param verticalVelocity Vertical velocity of the rocket (m/s)
  */
  void computeEstimatedApogee(
    float altitude,
  ...
  float calculateTerminalVelocity(float coefficientOfDrag, float airDensity) const;
  float calculateEstimatedApogee(float currentAltitude, float currentVertVelocity, float terminalVertVelocity);
